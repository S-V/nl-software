...
/**
  ...
  совпадают. В случае успешного завершения работы, функция возвращает \f$1\f$.
  Если матрица \f$A\f$ не является положительно определенной, то
  функция возвращает отрицательное значение \f$-k\f$, где \f$k\f$ ---
  номер первого неположительного углового минора.
  
  Для разложения используется алгоритм Холецкого (квадратных корней)
  
  Трудоемкость: \f$\frac{1}{3}n^3 + O(n^2)\f$ операций с плавающей запятой
*/
extern int chol_decomp(double* A, size_t n);
