...
/**
  ...
  связаны одномерный массив элементов \f$AN\f$ и два массива указателей:
  \f$JA\f$ и \f$IA\f$. Все ненулевые элементы хранятся построчно в массиве
  \f$AN\f$. Индексы столбцов ненулевых элементов - в массиве \f$JA\f$.
  Элементы массива \f$IA\f$ указывают на позиции, с которых начинается
  описание очередной строки. Более точно, описание \f$r\f$-й строки
  хранится в позициях с \f$IA[r]\f$ до \f$IA[r+1]-1\f$ массивов \f$JA\f$ и
  \f$AN\f$. Если \f$IA[r]=IA[r+1]\f$, то \f$r\f$-ая строка пустая. Отличие
  формата RR(C)O от RR(C)U в том, что при использовании первого формата
  ненулевые элементы внутри каждой строки храняться упорядоченно, при
  использовании второго ненулевые элементы внутри одной строки могут
  храниться в произвольном порядке.

  Для представления разреженных <i>симметричных</i> матриц используются
  RR(U)O и RR(U)U-форматы. Отличие этих форматов от двух предыдущих в том,
  что, в массивах \f$IA\f$, \f$JA\f$, \f$AN\f$ хранится информация об
  элементах матрицы, лежащих выше диагонали, диагональные элементы (в том
  числе нулевые) хранятся отдельно в одномерном массиве \f$AD\f$. 

  Подробно различные форматы хранения разреженных матриц опиcаны в 
  [ZolotykhBelov] и [Pissanezky]
*/
...
/**
  ...
  - Вход:  
     ...
     - \f$nz\f$ - число ненулевых элементов матрицы \f$A\f$
  - Выход: \f$IA\f$, \f$JA\f$, \f$AN\f$, \f$AD\f$ - массивы под представление 
    матрицы в RR(U)U-формате
*/
...
/**
  ...
  - Вход: 
         ...
         - \f$eps\f$ - элементы, меньшие \f$eps\f$ будут обнулены
  - Выход: 
         - \f$IA\f$, \f$JA\f$, \f$AN\f$, \f$AD\f$ - разреженное представление матрицы в 
           RR(U)O-формате
...
/**
  ...
  - Вход: 
         - \f$IA\f$, \f$JA\f$, \f$AN\f$ - разреженное представление матрицы в 
           RR(C)U-формате
         - \f$m\f$ - число строк матрицы \f$A\f$                     
         - \f$n\f$ - число столбцов матрицы \f$A\f$                  
  - Выход: 
...
/**
  Конвертация представления разреженной симметричной матрицы из формата RR(U)U в RR(C)U.
  ...
  - Вход: 
         - \f$IS\f$, \f$JS\f$, \f$SN\f$, \f$SD\f$ - разреженное представление матрицы в 
           RR(U)U-формате
         - \f$n\f$ - порядок матрицы
  - Выход: 
...
/**
  ...
  Вход:  \f$IA\f$, \f$JA\f$, \f$AN\f$, \f$AD\f$ - разреженное представление матрицы в 
         RR(U)U-формате \f$n\f$ - порядок матрицы \f$A\f$                  
...
/**
  ...
  - Вход: 
         ...
         - \f$m\f$ - число строк матрицы \f$A\f$
  - Выход: 
         - \f$IA\f$, \f$JA\f$, \f$AN\f$ - разреженное представление исходной матрицы в 
           RR(C)O-формате
...
/**
  ...
  Отличие от функции #sp_order - упорядочивается только портрет.
  - Вход: 
         - \f$IA\f$, \f$JA\f$ - разреженное представление матрицы в RR(C)U-формате
         - \f$m\f$ - число строк матрицы \f$A\f$
  - Выход: 
         - \f$IA\f$, \f$JA\f$ - разреженное представление исходной матрицы 
           в RR(C)O-формате

  Трудоемкость:
*/
extern void sp_order_profile(size_t* IA, size_t* JA, size_t m);
...
/**
  ...
  - Вход: 
         - \f$IA\f$, \f$JA\f$, \f$AN\f$, \f$AD\f$ - разреженное представление 
           симметричной матрицы в RR(U)U-формате
...
/**
  ...
  - Вход: 
         - \f$IA\f$, \f$JA\f$, \f$AN\f$, \f$AD\f$ - разреженное представление матрицы 
           в RR(U)U-формате
         - \f$n\f$ - число строк/столбцов матрицы \f$A\f$                  
         - \f$xformat\f$ - формат представления позиций элементов матрицы
         - \f$dformat\f$ - формат представления элементов матрицы
...
/**
  ...
  - Вход: 
         - \f$IA\f$, \f$JA\f$, \f$AN\f$ - разреженное представление матрицы 
           в RR(C)U-формате
         - \f$m\f$ - число строк матрицы \f$A\f$                     
         - \f$n\f$ - число столбцов матрицы \f$A\f$                  
  - Выход: 
         - \f$IAT\f$, \f$JAT\f$, \f$ANT\f$ - разреженное представление транспонированной 
           матрицы в RR(C)O-формате
...
/**
  ...
  - Вход: 
         ...
         - \f$sizeC\f$ - размер массива \f$JC\f$. Если размер не достаточен, 
                 функция возвращает \f$0\f$
  - Выход: 
         - \f$IC\f$, \f$JC\f$ - портрет суммы \f$A+B\f$ в RR(C)U-формате 
  - Результат:
         - \f$0\f$, если не хватило зарезервированной памяти, 
           и не \f$0\f$, если функция успешно завершилась.
  - Рабочий массив:
         - \f$work\f$ - массив длины \f$n\f$

  Трудоемкость:
*/
extern int sp_add_symb(size_t* IA, size_t* JA, size_t* IB, size_t* JB, size_t m, size_t n, size_t* IC, size_t* JC, size_t sizeC, size_t *xwork);
...
/**
  ...
  - Вход: 
         ...
         - \f$sizeC\f$ - размер массива \f$JC\f$. Если размер не достаточен, 
                 функция завершит работу и возвратит \f$0\f$
  - Выход: 
  ...
  Трудоемкость:
*/
extern int sp_mult_symb(size_t* IA, size_t* JA, size_t* IB, size_t* JB, size_t m, size_t k, 
  size_t* IC, size_t* JC, size_t sizeC, size_t *xwork);
...
/**
  ...
  - Вход:
         ...
         - \f$n\f$ - порядок матрицы
         - \f$IP\f$ - вектор перестановок
  - Выход:
         - \f$IB\f$, \f$JB\f$, \f$BN\f$, \f$BD\f$ - матрица \f$A\f$ в формате RR(U)U
           с переставленными строками и столбцами согласно перестановке \f$IP\f$
*/	
...
/**
  ...
  матрицы.
  Процедура строит символическое треугольное разложение симметричной
  матрицы A вида \f$A=U^{\rm T} D U\f$, где \f$U\f$ - верхняя треугольная
  матрица с единичной диагональю, \f$D\f$ - диагональная матрица.
  - Вход: 
         - \f$IA\f$, \f$JA\f$ - портрет матрицы \f$A\f$ в RR(U)U-формате
         - \f$n\f$ - порядок матрицы \f$A\f$
         - \f$sizeU\f$ - размер массива \f$JU\f$. Если размер не достаточен, 
                 функция возвратит нулевое значение
  - Выход: 
         - \f$IU\f$, \f$JU\f$ - портрет матрицы \f$U\f$ в RR(C)U-формате
           (диагональ не хранится)
  - Результат:
         \f$0\f$, если не хватило зарезервированной памяти, 
          и не \f$0\f$, если функция успешно завершилась.
  - Рабочий массив:
  ...
  Трудоемкость:
*/
extern int sp_chol_symb(size_t* IA, size_t* JA, size_t n, size_t* IU, size_t* JU, size_t sizeU,
  size_t *xwork);
...
/**
  ...
  положительно определенной матрицы \f$A\f$ вида \f$A=U^{\rm T}U\f$, где \f$U\f$ - верхняя треугольная матрица с единичной диагональю,
  \f$D\f$ - диагональная матрица.
  - Вход: 
         - \f$IA\f$, \f$JA\f$, \f$AN\f$, \f$AD\f$ - разреженное представление матрицы \f$A\f$ 
           в RR(U)O-формате
         - \f$IU\f$, \f$JU\f$ - портрет матрицы \f$U\f$ в RR(U)O-формате
         - \f$n\f$ - порядок матрицы \f$A\f$
  - Выход: 
  ...
  Обратим внимание, что матрица $A$ и портрет матрицы $U$ должны быть заданы
  в упорядоченном (RR(U)O) формате, тогда как функция #sp_chol_num принимает на вход
  и возвращает матрицы в неупорядоченном (RR(U)O) формате. Для конвертации форматов используйте
  #sp_order и #sp_order_profile.

  Трудоемкость:
*/
extern void sp_chol_num(size_t* IA, size_t* JA, double* AN, double* AD, size_t* IU, size_t* JU, 
  size_t n, double* UN, double* DINV);

/**
  Решение симметричной разреженной системы линейных уравнений \f$Ax=b\f$ методом
  \f$A=U^{\rm T}U\f$-разложения (Холецкого).
  - Вход: 
         - \f$IU\f$, \f$JU\f$, \f$UN\f$ - разреженное представление верхней треугольной матрицы
           \f$U\f$ с единичной диагональю в RR(U)U-формате (диагональ не хранится)
         - \f$DINV\f$ - значения диагональных элементов матрицы, обратной к \f$D\f$
...
/**
  ...
  - Вход: 
         - \f$IA\f$, \f$JA\f$, \f$AN\f$, \f$AD\f$ - разреженное представление матрицы \f$A\f$ в 
            RR(LU)U-формате
...
/**
  Решение разреженной системы линейных уравнений \f$Ax=b\f$ методом Гаусса-Зейделя 
  (с выбором параметра релаксации). Отличие от функции #sp_gauss_seidel в том,
  что для представления матрицы \f$A\f$ используется обычный RR(C)U-формат.
  - Вход: 
         - \f$IA\f$, \f$JA\f$, \f$AN\f$ - разреженное представление матрицы \f$A\f$ в RR(C)U-формате
         - \f$b\f$ - вектор правых частей системы
         - \f$n\f$ - порядок системы
  ...
  Трудоемкость:
*/
extern int sp_gauss_seidel_complete(size_t* IA, size_t* JA, double* AN, double* b, size_t n, double f, double eps, int max_iter, double* x, double *work);
...
/**
  ...
  - Вход:
        - \f$IA\f$, \f$JA\f$, \f$AN\f$, \f$AD\f$ - разреженное представление симметричной 
          матрицы \f$A\f$ в RR(U)U-формате
        - \f$b\f$ - вектор правых частей системы
        - \f$n\f$ - порядок системы
        - \f$eps\f$ - заданная точность, с которой необходимо вычислить решение системы
        - \f$max\_iter\f$ - максимальное количество итераций  
