...
/**
  ...
  \f$work\f$ - рабочий массив длины \f$n\f$ (используется для оценки числа обусловленности)
  
  Трудоемкость: \f$\frac{2}{3}n^3 + O(n^2)\f$ операций с плавающей запятой
*/
extern double lu_decomp(double* A, size_t n, size_t* p, int *sgn, double *work);
...
/**
  ...
  Функция выполняет один шаг итерационного уточнения решения \f$x\f$
  системы линейных уравнений \f$Ax=b\f$. Используется предварительно
  найденное \f$LU\f$-разложение матрицы \f$A\f$, представленное массивом
  \f$LU\f$ и вектором перестановок \f$p\f$, возвращаемыми функцией
  #lu_decomp. Возвращается улучшенное решение \f$x\f$ и вектор невязки
  \f$r=Ax-b\f$.

  Используется следующий алгоритм: вначале вычисляется вектор
  \f$r'=Ax-b\f$, после этого с помощью функции #lu_solve решается система
  \f$Ar'=b\f$ и, наконец, определяется улучшенное решение \f$x=x-r\f$ и
  вектор невязки \f$Ax-b\f$.
...
/**
  ...
  Трудоемкость: \f$2n^2 + O(n)\f$ операций с плавающей запятой
*/
extern void lu_invert(double* LU, size_t n, size_t* p, double* B, double *work);
